use db_microservisio_reporte_sensor_mb;

CREATE OR REPLACE VIEW resumen_evento_organizador AS
...
eventos_organizadores AS (
    SELECT
        e.id as evento_id,
        e.nombre as evento_nombre,
        e.fecha_apertura as evento_fecha,
        org.nombre as organizador,
        org.numero_documento as organizador_numero_documento
    FROM eventos e
    JOIN organizadores_eventos oe ON e.id = oe.eventos_id
    JOIN organizadores org ON org.numero_documento = oe.organizadores_numero_documento
)
SELECT
    -- ID artificial para JPA
    ROW_NUMBER() OVER (ORDER BY eo.evento_id, eo.organizador_numero_documento) as id,

    -- Datos del evento y organizador
    eo.evento_id,
    eo.evento_nombre as nombre,
    eo.evento_fecha as fecha_evento,
    eo.organizador_numero_documento,
    eo.organizador,
    ...
    -- Comisión AllTickets corregida (usando total_recaudado)
    COALESCE((SUM(CASE WHEN bt.ticket_estado = 1 THEN bt.precio + bt.servicio + bt.iva END) * cd.comision_at), 0) * 1.19 as comision_alltickets,
...
LEFT JOIN base_tickets bt ON bt.evento_id = eo.evento_id
GROUP BY eo.evento_id, eo.evento_nombre, eo.organizador_numero_documento, cd.comision_at, cd.comision_pasarela_valor, cd.comision_3ds_valor
ORDER BY eo.evento_id, eo.organizador_numero_documento;