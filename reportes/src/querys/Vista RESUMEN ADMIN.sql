...
SELECT
    ...
    -- Métricas del día (cuando la fecha coincide con la fecha del registro)
    COUNT(DISTINCT CASE
        WHEN bt.orden_estado = 1 AND bt.ticket_estado = 1 AND bt.orden_tipo != 5
        AND bt.fecha = CURDATE() THEN bt.ticket_id END) as tickets_vendidos_hoy,

    COALESCE(SUM(CASE
        WHEN bt.orden_estado = 1 AND bt.ticket_estado = 1 AND bt.orden_tipo != 5
        AND bt.fecha = CURDATE()
...
WHERE bt.fecha IS NOT NULL  -- Solo incluir registros con fechas válidas
GROUP BY e.id, e.nombre, bt.fecha, bt.anio, bt.mes, bt.dia, cd.comision_at, cd.comision_pasarela_valor, cd.comision_3ds_valor
ORDER BY e.id, bt.fecha;