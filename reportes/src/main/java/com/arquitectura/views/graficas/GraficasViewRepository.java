 ...
 */
@Repository
public interface GraficasViewRepository extends JpaRepository<GraficaView, Long> {

    /**
     * Obtiene datos para gráfica de líneas
     *
     * Funcionalidad:
    ...
    @Query(value = """
        WITH periodos AS (
            ...
            SELECT CASE WHEN :mes = -1 THEN mes ELSE dia END as periodo,
                   ...
                   SUM(total_tickets) as total_asistentes,
                   SUM(total_precio) as total_precio_recaudado, 
                   SUM(total_servicio) as total_servicio_recaudado,
                   SUM(total_iva) as total_iva_recaudado, 
        ...
        SELECT p.numero as periodo,
               CASE WHEN :mes = -1 THEN 
                   CASE p.numero WHEN 1 THEN 'Enero' 
                   WHEN 2 THEN 'Febrero' 
                   WHEN 3 THEN 'Marzo' 
                   WHEN 4 THEN 'Abril'
                   WHEN 5 THEN 'Mayo' 
               ...
               COALESCE(d.total_recaudado, 0) as recaudado, 
               COALESCE(d.total_asistentes, 0) as asistentes,
               COALESCE(d.total_precio_recaudado, 0) as precioRecaudado, 
               COALESCE(d.total_servicio_recaudado, 0) as servicioRecaudado,
               COALESCE(d.total_iva_recaudado, 0) as ivaRecaudado, 
               COALESCE(d.total_recaudado_transacciones, 0) as recaudadoTrx
        ...
        ORDER BY p.numero
        """, nativeQuery = true)
    List<Object[]> getGraficaLineaVentas(@Param("eventoId") Long eventoId,
    ...
    @Query(value = """
        ...
        SELECT mb.metodo as metodo, COALESCE(dr.total_recaudado, 0) as totalRecaudado,
    ...
    /**
     * Obtiene datos para gráfica de líneas globales (todos los eventos)
     *
     * Funcionalidad:
     * - Si eventoId = null: Suma TODOS los eventos
     * - Si eventoId != null: Comportamiento igual al método original
     * - Si mes = -1: Retorna todos los meses del año (1-12) aunque no tengan ventas
     * - Si mes != -1 Y dia = -1: Retorna todos los días del mes específico aunque no tengan ventas
     * - Si mes != -1 Y dia != -1: Retorna todas las horas del día específico (0-23) aunque no tengan ventas
     * - Períodos sin ventas aparecen con valores en 0
     * - Nombres de períodos en español (Enero, Lunes, 00:00, etc.)
     *
     * @param eventoId ID del evento a consultar o null para todos los eventos
     * @param mes Mes específico (1-12) o -1 para consultar todo el año
     * @param dia Día específico (1-31) o -1 para consultar todo el mes
     * @param anio Año específico a consultar (formato YYYY)
     * @return Lista de GraficaLineasDTO con datos temporales completos
     *
     */
    @Query(value = """
WITH periodos AS (
    SELECT 0 as numero UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 
    UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12
    UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18
    UNION SELECT 19 UNION SELECT 20 UNION SELECT 21 UNION SELECT 22 UNION SELECT 23 UNION SELECT 24
    UNION SELECT 25 UNION SELECT 26 UNION SELECT 27 UNION SELECT 28 UNION SELECT 29 UNION SELECT 30 UNION SELECT 31
    ...
    SELECT CASE 
           ...
           SUM(total_recaudado_transacciones) as total_recaudado_transacciones
    FROM grafica_lineas 
    WHERE (:eventoId IS NULL OR evento_id = :eventoId) 
        AND anio = :anio 
        AND (:mes = -1 OR mes = :mes)
        AND (:dia = -1 OR dia = :dia)
    GROUP BY CASE 
                 WHEN :mes = -1 THEN mes 
                 WHEN :dia = -1 THEN dia 
                 ELSE HOUR(fecha) 
...
SELECT p.numero as periodo,
       CASE 
           ...
           WHEN :mes = -1 THEN 
               ...
               END
           -- Vista mensual: nombres de días de semana
           WHEN :dia = -1 THEN 
               CASE DAYNAME(CONCAT(:anio, '-', LPAD(:mes, 2, '0'), '-', LPAD(p.numero, 2, '0')))
               ...
               END
           -- Vista diaria: formato de horas
           ELSE CONCAT(LPAD(p.numero, 2, '0'), ':00')
       END as nombrePeriodo,
       COALESCE(d.total_recaudado, 0) as recaudado, 
       COALESCE(d.total_asistentes, 0) as asistentes,
       COALESCE(d.total_precio_recaudado, 0) as precioRecaudado, 
       ...
       COALESCE(d.total_recaudado_transacciones, 0) as recaudadoTrx
FROM periodos p 
LEFT JOIN datos_vista d ON p.numero = d.periodo
WHERE CASE 
          WHEN :mes = -1 THEN p.numero >= 1 AND p.numero <= 12
          WHEN :dia = -1 THEN p.numero >= 1 AND p.numero <= DAY(LAST_DAY(CONCAT(:anio, '-', LPAD(:mes, 2, '0'), '-01')))
          ELSE p.numero >= 0 AND p.numero <= 23
      END
ORDER BY p.numero
""", nativeQuery = true)
    List<Object[]> getGraficaLineaVentasAdmin(@Param("eventoId") Long eventoId,
                                              @Param("mes") Integer mes,
                                              @Param("dia") Integer dia,
                                              @Param("anio") Integer anio);

    /**
     * Obtiene datos para gráfica de dona de recaudación por método de pago globales
     *
     * Funcionalidad:
     * - Si eventoId = null: Suma TODOS los eventos
     * - Si eventoId != null: Comportamiento igual al método original
     ...
     * - Métodos sin ventas aparecen con valores en 0
     * - Incluye dos tipos de recaudado: por tickets y por transacciones
     * - Si mes = -1: Agrupa todo el año
     * - Si mes != -1 Y dia = -1: Solo ese mes
     * - Si mes != -1 Y dia != -1: Solo ese día específico
     *
     * @param eventoId ID del evento a consultar o null para todos los eventos
     * @param mes Mes específico (1-12) o -1 para consultar todo el año
     * @param dia Día específico (1-31) o -1 para consultar todo el mes
...
SELECT mb.metodo as metodo, 
       COALESCE(dr.total_recaudado, 0) as totalRecaudado,
       COALESCE(dr.total_recaudado_transacciones, 0) as totalRecaudadoTransacciones
FROM metodos_base mb 
LEFT JOIN datos_reales dr ON mb.metodo = dr.metodo_pago
ORDER BY mb.metodo
""", nativeQuery = true)
    List<GraficaDonaDTO> graficaRecaudadoByMetodoAdmin(@Param("eventoId") Long eventoId,
                                                       @Param("mes") Integer mes,
                                                       @Param("dia") Integer dia,
                                                       @Param("anio") Integer anio);
}